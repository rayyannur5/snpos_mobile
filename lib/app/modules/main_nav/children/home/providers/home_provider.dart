...
class HomeProvider extends BaseApiProvider {
  ...
  Future<Response> getListProducts(String token) async {
    Response response = await get(
  ...
  Future<Response> sendListProducts(transaction, items, String token) async {
    final imageFile = transaction['path_picture'] != '' && transaction['path_picture'] != null
        ? File(transaction['path_picture'])
        : null; // ambil file dari path
